...
# with open('hierar_sentiment.model', 'wb') as output:
#     pickle.dump(model1, output, pickle.HIGHEST_PROTOCOL)
#
# with open('hierar_sentiment.model', 'wb') as output:
#     pickle.dump([model1, vectorizer,word_polarity], output, pickle.HIGHEST_PROTOCOL)

model1,vectorizer,word_polarity=pickle.load(open('hierar_sentiment.model', 'rb'))



ii = remove_high_sentiment_words(copy.copy(['shobhit is a goof boy']), postiveTestThreshold, negativeTestThreshold,
                                               word_polarity)

print model1.predict(vectorizer.transform(ii))
